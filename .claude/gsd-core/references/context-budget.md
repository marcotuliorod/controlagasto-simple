...
The keys live in `.claude/settings.json` (project) or `/Users/marcotuliorodgmail.com/Library/Mobile Documents/com~apple~CloudDocs/Projetos/Controla_Gastos_Simples/controlagasto-simple/.claude/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/marcotuliorodgmail.com/Library/Mobile Documents/com~apple~CloudDocs/Projetos/Controla_Gastos_Simples/controlagasto-simple/.claude/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
