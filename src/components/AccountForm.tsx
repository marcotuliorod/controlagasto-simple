...
// Extracted so the fields can be embedded directly (e.g. inside the
// onboarding wizard) without nesting a Dialog inside another container.
export const AccountFormFields = ({ account, onSubmit, onCancel, cancelLabel, submitLabel, submitDisabled }: AccountFormFieldsProps) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-2 justify-end pt-4">
        ...
        <Button type="submit" disabled={submitDisabled}>
          {submitLabel ?? (account ? "Salvar" : "Criar")}
        </Button>
      </div>
    </form>
  );
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <AccountFormFields account={account} onSubmit={handleSubmit} onCancel={onClose} />
      </DialogContent>
    </Dialog>
  );
};
